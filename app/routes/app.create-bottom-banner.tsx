...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "create") {
    ...
    // Create bottom banner with slides
    const bottomBanner = await (prisma as any).bottomBanner.create({
      data: {
        isActive,
        bannerWidth,
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      const slideData = {
        bottomBannerId: bottomBanner.id,
        order: i,
...
// ---- UI ----
export default function CreateBottomBannerPage() {
  ...
  const [slides, setSlides] = useState<BottomBannerSlide[]>([defaultSlide])
  ...
  const [productFontSize, setProductFontSize] = useState(defaultSettings.productFontSize)
  const [actionButtonTextColor, setActionButtonTextColor] = useState(defaultSettings.actionButtonTextColor)
  const [actionButtonBackgroundColor, setActionButtonBackgroundColor] = useState(defaultSettings.actionButtonBackgroundColor)
  const [actionButtonBorderRadius, setActionButtonBorderRadius] = useState(defaultSettings.actionButtonBorderRadius)
  const [actionButtonPadding, setActionButtonPadding] = useState(defaultSettings.actionButtonPadding)
  const [addToCartButtonTextColor, setAddToCartButtonTextColor] = useState(defaultSettings.addToCartButtonTextColor)
  const [addToCartButtonBackgroundColor, setAddToCartButtonBackgroundColor] = useState(defaultSettings.addToCartButtonBackgroundColor)
  const [viewProductButtonTextColor, setViewProductButtonTextColor] = useState(defaultSettings.viewProductButtonTextColor)
  const [viewProductButtonBackgroundColor, setViewProductButtonBackgroundColor] = useState(defaultSettings.viewProductButtonBackgroundColor)
  ...
  return (
    <Page>
      <TitleBar title="Create New Bottom Banner" />
      <Layout>
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <Form method="post" onSubmit={handleSubmit}>
                ...
                <BlockStack gap="400">
                  ...
                  <InlineStack gap="200">
                    ...
                    >
                      {isSubmitting ? "Creating Bottom Banner..." : isRedirecting ? "Redirecting..." : "Create Bottom Banner"}
                    </Button>
